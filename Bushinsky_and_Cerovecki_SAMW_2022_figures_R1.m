...
% also save out data for Table 2
...
marker_size = 3;
% disp(' Note - currently delta pCO2 is not calculated the same for the o2 and')
% pCO2 - pCO2 is surface only, O2 is a mean over ML or upper 30 m (check)
...
clear x1 x1_pos d p eA pA argo_index plot_count 
...
%% ??? Figure 7 - BSOSE property variability in time

% % left column - anomalies
% % right column - relationship between properties and density relative to
% % the overall bSOSE pacific
% 
...
% 
% var_names = fieldnames(bSOSE_out.(reg_names{1}));
% var_names = var_names([1:5 7]);
% 
% for r = 1:length(reg_names)
%     for v = 1:length(var_names)
%         
%         % find values between NaNs - these correspond to the values for
%         % each year - regionprops is a function in the image processing
%         % toolbox
%         temp = regionprops(~isnan(bSOSE_out.(reg_names{r}).(var_names{v})), bSOSE_out.(reg_names{r}).(var_names{v}), 'PixelValues');
%         
%         % initialize a vector for annual averages
%         bSOSE_out.(reg_names{r}).avg.(var_names{v}) = NaN(length(temp),1);
%         
%         % calculate averages
%         for t = 1:length(temp)
%             bSOSE_out.(reg_names{r}).avg.(var_names{v})(t) = nanmean(temp(t).PixelValues);
%         end
%     end
% end
...
for r=1:2
    ...
    for pl = 1:length(plot_map)
        ...
        if pl==1
            %             title([regions{r} ' ' properties{p} ' ' num2str(mw_prop.(regions{r}).all.mean_prop_weighted.(properties{p})(1),4) ' +/- ' num2str(mw_prop.(regions{r}).all.mean_prop_weighted.(properties{p})(2),3) ], 'interpreter', 'none')
            %             title(upper(regions{r}))
            
        else
            %             title([properties{p} ' ' num2str(mw_prop.(regions{r}).all.mean_prop_weighted.(properties{p})(1),4) ' +/- ' num2str(mw_prop.(regions{r}).all.mean_prop_weighted.(properties{p})(2),3) ], 'interpreter', 'none')
            %
        end
...
print(gcf, '-dpdf', [plot_dir plot_filename '.pdf' ])
% bSOSE_out.CPac_200 = load([cerovecki_dir '/Data_from_Ivana/2021_10_05/' 'MLD_av_8_9_CPac_200_m_025_ST_iter135_45_64_170_246_c_pacific.mat']);
% bSOSE_out.SEPac_200 = load([cerovecki_dir '/Data_from_Ivana/2021_10_05/' 'MLD_av_8_9_SEPac_200_m_025_ST_iter135_45_64_246_290_se_pacific.mat']);
% bSOSE_out.Pac_200 = load([cerovecki_dir '/Data_from_Ivana/2021_10_05/' 'MLD_av_8_9_Pac_200_m_025_ST_iter135_45_64_170_290_pacific.mat']);

% sose.pacific = load([cerovecki_dir 'Data_from_Ivana/2021_08/BIN_MLD_CLIM_iter135_Pac_200_m_05_ST_lat_45_64_lon_170_290_regional_pacific']);
bsose_years = 2013:2019;

plot_filename = 'Fig_7 BSOSE_regional_property_time_series_v4';
plot_colors = brewermap(3, 'Set1');
plot_colors (3,:) = [0 0 0];

plot_symbols = {'x', 'o', 'd'};
% var_names = {'NO3_core_c_pacific_200' 'NO3_core_se_pacific_200' 'NO3_core_pacific_200'; ...
%     'DIC_core_c_pacific_200' 'DIC_core_se_pacific_200' 'DIC_core_pacific_200'; ...
%     'O2_core_c_pacific_200' 'O2_core_se_pacific_200' 'O2_core_pacific_200'; ...
%     'rho_core_c_pacific_200' 'rho_core_se_pacific_200' 'rho_core_pacific_200';...
%     'T_core_c_pacific_200'   'T_core_se_pacific_200' 'T_core_pacific_200'  ; ...
%     'vol_core_c_pacific_200' 'vol_core_se_pacific_200' 'vol_core_pacific_200' };
...
for z = [5 1 2 3]
    if z==2
        sose_prop = 'no3_mld_clim';
        prop_name = '[NO_3^-] (\mumol kg^-^1)';
        anomaly_text = '[NO_3^-] anomaly';
    elseif z==1
        sose_prop = 'dic_mld_clim';
        prop_name = '[DIC] (\mumol kg^-^1)' ;
        anomaly_text = '[DIC] anomaly';
        
    elseif z==3
        sose_prop = 'o2_mld_clim';
        prop_name = '[O_2] (\mumol kg^-^1)' ;
        anomaly_text = '[O_2] anomaly';
        
    elseif z==4
        sose_prop = 'R_mld_clim';
    elseif z==5
        sose_prop = 'T_mld_clim';
        prop_name =  '\theta (\circC)';
        anomaly_text = '\theta anomaly';
        
    elseif z==6
        sose_prop = 'vol_mld_clim';
    end
    ...
    hold on
    %     plot([2013 2019], [0 0], '--k')
    l_n = NaN(size(var_names,2),1);
    
    for q = 1:size(reg_names,1)
        
        %         l_n(q) = plot(bsose_years, bSOSE_out.(reg_names{q}).avg.(var_names{z}) - nanmean(bSOSE_out.(reg_names{q}).avg.(var_names{z})), 'color', plot_colors(q,:), 'linewidth', 2);
        l_n(q) = plot(bsose_years, bSOSE_out.(reg_names{q}).avg.(var_names{z}), 'color', plot_colors(q,:), 'linewidth', 2);
    ...
    subplot(4,2,plot_count);
    rsq = NaN(size(reg_names,1),1);
    corr_coefs = NaN(size(reg_names,1),1);
    l_n = NaN(size(reg_names,1),1);
    for q = 1:length(reg_names)
        
        
        
        % plot bSOSE mean value as plotted in Fig. 2
        plot(nanmean(sose.pacific.R_mld_clim(5:end,month_lims_for_mean(1):month_lims_for_mean(length(month_lims_for_mean))),2), ...
            nanmean(sose.pacific.(sose_prop)(5:end,month_lims_for_mean(1):month_lims_for_mean(length(month_lims_for_mean))),2), 'color', b_sose_color, 'linestyle', '--', 'linewidth', 3)
        
        
        % x_vals = min(CPac_200.vol_core_c_pacific_200):10e13:max(CPac_200.vol_core_c_pacific_200);
        
        %         x = bSOSE_out.(reg_names{q}).avg.(var_names{4,q})';
        %         y = bSOSE_out.(reg_names{q}).avg.(var_names{z,q})';
        %
        x = bSOSE_out.(reg_names{q}).avg.(var_names{6})';
        y = bSOSE_out.(reg_names{q}).avg.(var_names{z})';
        % [m, bint, resid, rint,stats] = regress(y, [x ones(length(y),1)]);
        %
        %
        %     fit2 = m(1).*x_vals+m(2);
        [p, ~, mu] = polyfit(x, y, 1);
        ...
        rsq(q) = 1 - SSresid/SStotal;
        corr_coefs(q) = correlate(y, x);
        %     plot(enso_vals, fit, '-r')
        l_n(q) = plot(x, f, '-', 'color', plot_colors(q,:), 'linewidth', 3);
        hold on
        plot(x, y, plot_symbols{q}, 'linewidth',2, 'color', plot_colors(q,:))
        
        clear SStotal SSresid yresid f y x p mu
    end
    if plot_count==2
        %         legend(l_n, 'C Pac', 'SE Pac', 'Pac', 'location', 'Southwest', 'fontsize', 11)
    end
    set(gca, 'xlim', [26.88 26.98])
    
    %     t1 = title(['C Pac R^2: ' num2str(rsq(1),2) ' SE Pac R^2: ' num2str(rsq(2),2) ' Pac R^2: ' num2str(rsq(3),2)]);
    %     title_pos = get(t1, 'position');
    %     set(t1, 'position', title_pos+[0 diff(get(gca, 'ylim')).*.1 0])
    
    %     text(min(get(gca,'xlim')), max(get(gca,'ylim')) + diff(get(gca, 'ylim')).*.05, ...
    %         ['C Pac Corr: ' num2str(corr_coefs(1),2) ' SE Pac Corr: ' num2str(corr_coefs(2),2) ' Pac Corr: ' num2str(corr_coefs(3),2)], 'fontsize', 14)
    set(gca, 'fontsize', 13)
    ylabel(prop_name)
    xlabel(    '\sigma_\theta (kg m^-^3)')
    clear rsq corr_coefs l_n
...
set(l1, 'position', l1_pos+[.01 .01 0 0])
%
% print(gcf, '-dpng', '-r300', [cerovecki_dir 'plots/bSOSE_comparisons/IAV/' plot_filename '.png' ])
print(gcf, '-dpdf', '-r800',  [plot_dir plot_filename '.pdf'])

clear z q m t1 title_pos
...
hold on
grid on
% plot(diff_table(:,1), diff_table(:,2), 'xk')
% for d=1:length(diff_table)
%    % plot uncertainties on SOCAT
%     plot([diff_table(d,1) - diff_table(d,15) diff_table(d,1) + diff_table(d,15)], [diff_table(d,2) diff_table(d,2)],'-k')
%     % plot uncertainties on SOCCOM
%     plot([diff_table(d,1) diff_table(d,1)], [diff_table(d,2) - 10 diff_table(d,2) + 10],'-k')
%
% end

socat_minus_argo = diff_table(:,1) - diff_table(:,2);

% scatter(diff_table(:,1), diff_table(:,2), 50, diff_table(:,9)-diff_table(:,16), 'filled', 'marker', 's','markeredgecolor', 'k')
% c1  =colorbar;
% ylabel(c1, 'Days since float deployment')
...
% title(['SOCAT v2020 prelim crossover comparison within +/-: ' num2str(day_range) ' days and '  num2str(distance_range) ' km'])
set(gca, 'ylim', [250 440], 'xlim', [250 440])
d2 = subplot(1,2,2);
% h1 = histogram(socat_minus_argo,10);
% h1.FaceColor = cmap(2,:);
% h1.BinWidth=10;
hold on
plot([0 0], get(gca, 'ylim'), '--k')
% legend(h1, ['n: ' num2str(sum(~isnan(socat_minus_argo)))])
% title(['Mean: ' num2str(nanmean(socat_minus_argo),3) ', std: ' num2str(nanstd(socat_minus_argo),3)])
% xlabel('SOCAT minus Float pCO_2')
% print(gcf,'-dpng', '-r300', [Plot_dir plot_filename '_time_since_dep.png'])
...
%
cla(gca)
% histogram([(diff_table(:,1) - diff_table(:,2))' ; diff_filtered_dens_difference'])
% h1= histogram(diff_table(:,1) - diff_table(:,2),10);
%
hold on
...
title('Differences')
% legend([h1 h2], ['n: ' num2str(sum(~isnan(socat_minus_argo)))], ['n: ' num2str(sum(~isnan(diff_filtered_dens_difference)))])
...
% title(['Green: remove > ' num2str(density_threshold) 'kg/m3 diff. Mean filtered: ' num2str(nanmean(diff_filtered_dens_difference),3) ', std: ' num2str(nanstd(diff_filtered_dens_difference),3)])
%
print(gcf,'-dpdf', '-r300', [plot_dir plot_filename 'density_comp ' num2str(density_threshold) '.pdf'])
...
for r = 1:size(reg_names,2)
    ...
    %     x = bSOSE_out.(reg_names{q}).(var_names{5,q})';
    %     y = bSOSE_out.(reg_names{q}).(var_names{3,q})';
...
% dir_my_scripts=['/data1/project/gyre/BSOSE/scripts/iter135/Hovmuller/'];
% data_mm=['/data2/project/gyre-data/BSOSE/iter135/MonthMean_NEW/'];
